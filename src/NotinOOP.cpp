...
void NotinOOP::loadData()
{
    ...
    while (std::getline(usersFile, line))
    {
        ...
        else if (userType == "B")
        {
            ...
            buyer = new Buyer(id, username, password, balance, reviewsRemoved);

            // wishlist
            std::string wishlistStr;
            std::getline(ss, wishlistStr, '|');

            std::stringstream wishlistSS(wishlistStr);
            ...
            while (wishlistSS >> fragranceIDStr)
            {
                ...
                Fragrance *foundFragrance = nullptr;
                for (size_t i = 0; i < catalogue.size(); i++)
                {
                    if (catalogue[i].getID() == targetID)
                    {
                        foundFragrance = &catalogue[i];
                        break;
                    }
                }
                ...
                if (foundFragrance != nullptr)
                {
                    buyer->addToWishlist(*foundFragrance);
                }
                else
                {
                    std::cout << "Warning: Fragrance ID " << targetID << " found in user wishlist but missing from catalogue!" << std::endl;
                }
            }

            // cart
            std::string cartStr;
            std::getline(ss, cartStr, '|');
            ...
            while (cartSS >> cartFragranceIDStr)
            {
                ...
                size_t targetID = std::stoull(cartFragranceIDStr);
                ...
                if (foundFragrance != nullptr)
                {
                    buyer->addToCart(*foundFragrance);
                }
                else
                {
                    std::cout << "Warning: Fragrance ID " << targetID << " found in cart but missing from catalogue!" << std::endl;
                }
            }

            // purchases  <purchaseID:<fragranceIDs>:fragranceID:status:userID:finalPrice>

            std::string purchasesStr;
            std::getline(ss, purchasesStr, '|');

            std::stringstream purchasesSS(purchasesStr);
            std::string purchaseInfoStr;
            ...
            while (purchasesSS >> purchaseInfoStr)
            {
                ...
                while (fragrancesSS >> fragranceIDStr)
                {
                    ...
                    if (foundFragrance != nullptr)
                    {
                        purchaseFragrances.push_back(*foundFragrance);
                    }
                    else
                    {
                        std::cout << "Warning: Fragrance ID " << targetID << " found in purchase but missing from catalogue!" << std::endl;
                    }
                }
                ...
                {
                    std::cout << "Warning: Invalid purchase status found in purchases! Defaulting to PENDING." << std::endl;
                    purchaseStatus = PurchaseStatus::PENDING;
                }

                Purchase loadedPurchase(purchaseID, purchaseFragrances, purchaseStatus, userID, finalPrice);
                buyer->addToPurchases(loadedPurchase);
            }

            // discounts <type:discountID:percent:brand/bonus>

            std::string discountsStr;
            std::getline(ss, discountsStr, '|');

            std::stringstream discountsSS(discountsStr);
            std::string discountInfoStr;
            ...
            while (discountsSS >> discountInfoStr)
            {
                ...
                {
                    std::cout << "Warning: Invalid discount type found in discounts! Skipping this discount." << std::endl;
                }
            }
